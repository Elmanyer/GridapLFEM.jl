# ==============================================================
#  test_conservation_alg.jl — mass conservation (SMALL & FAST)
#
...
println("=" ^ 60)
println("  test_conservation_alg.jl — mass conservation (closed basin)")
println("=" ^ 60)
...
U, V = build_fe_spaces_alg(model, fe_order, Nσ; y_wall_bc=true, x_wall_bc=true)

prob = build_problem_alg(vert; g=g, d_func=x -> d_val,
    linearised=true, advection=true,
    mu_sponge=x -> 0.0, wm_src=(x, t) -> 0.0)

A0 = 0.01; xc = 5.0; wd = 1.0
eta0(x) = A0 * exp(-((x[1] - xc)^2) / wd^2)
u0 = make_initial_conditions_alg(U, Nσ; eta0_func=eta0)
...
solver = build_ode_solver_alg(0.02)
...
println("=" ^ 60)
n_fail > 0 ? error("test_conservation_alg: $n_fail failed!") :
