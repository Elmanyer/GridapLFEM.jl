# ==============================================================
#  test_nlpressure_alg_distributed.jl — distributed nl_pressure_full smoke
#                                       (4 ranks, 2×2)
#
#  Validates the distributed frozen-projection mass solve
#  (nlpressure_alg.jl :: build_nlp_ctx(...; distributed=true) →
#  CGSolver(JacobiLinearSolver())) by running the SAME tanh-bar case as gate
#  G3 of test_nlpressure_alg.jl (ALL pressure flags on) both distributed and
...
# sequential reference (same config, run once via setup_and_run_alg): see
# test_nlpressure_alg.jl gate G3 — bar run, 40 steps.
...
if is_rank0
    ...
    println("=" ^ 60)
    println("  test_nlpressure_alg_distributed.jl — results (4 ranks, 2×2)")
    println("=" ^ 60)
    ...
    flush(stdout)
    n_fail > 0 && error("test_nlpressure_alg_distributed: $n_fail failed!")
