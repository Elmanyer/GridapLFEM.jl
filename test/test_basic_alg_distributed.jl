# ==============================================================
#  test_basic_alg_distributed.jl — distributed smoke test (4 ranks, 2×2)
#
#  Same tiny 16×2 flat-bed flume as test_basic_alg.jl, run distributed with
...
#  reference amplitudes from test_basic_alg.jl (GMRES vs LU ⇒ loose 1e-3 rel).
#
...
if is_rank0
    println("=" ^ 60)
    println("  test_basic_alg_distributed.jl — results (4 ranks, 2×2)")
    println("=" ^ 60)
    ...
    flush(stdout)
    n_fail > 0 && error("test_basic_alg_distributed: $n_fail failed!")
