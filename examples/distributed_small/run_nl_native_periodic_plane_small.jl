# ==============================================================
#  run_nl_native_periodic_plane_small.jl — SMALL-DOMAIN comparison run
#
#  CASE 8/12:  nonlinear · NATIVE nonlinear pressure · flat bed · y-periodic · A=0.1
#  Same flat periodic plane wave as cases 2 & 7, with nl_pressure=:native — the
#  exact, quasi-Newton native tier 𝓝{3,6,7,8} (the Class-I outer products + the
#  Class-II component 3), WITHOUT the frozen-projection Class-III {1,2,4,5}.
#  Middle of the pressure sweep {none (7) → native (8) → full (2)}.
#
#  Geometry identical to case 2. d=3.5, T=2.0 ⇒ kd≈3.5, λ≈6.25 m, c≈3.13 m/s.
#
#  LAUNCH:  LFEM_PX=4 LFEM_PY=1 mpiexecjl --project=. -n 4 julia --project=. \
#             GridapLFEM.jl/examples/distributed_small/run_nl_native_periodic_plane_small.jl
#  (SLURM: run/dist_small/run_nl_native_periodic_plane_small.sh)
# ==============================================================

include(joinpath(@__DIR__, "..", "distributed", "_dist_common.jl"))

# ---- CASE 8 physics config (env still overrides; keeps banner ⇔ solver consistent) ----
get!(ENV, "LFEM_REGIME", "nonlinear"); get!(ENV, "LFEM_NL_PRESSURE", "native")
...
outdir   = genv("LFEM_OUTDIR", joinpath(ROOT, "output", "small_nl_native_periodic_plane_M$(M)"))

banner("SMALL | NONLINEAR native-𝓝-pressure periodic plane wave (flat bed, A=$(Awave))",
...
diags, vert, prob = setup_and_run_distributed(
    ...
    regime=regime_sym(), nl_pressure=nl_pressure_sym(), flat_bed=flat_bed_flag(),  # ★ CASE 8 (defaults above)
...
is_rank0() && @printf("small_nl_native_periodic_plane done: %d steps, %d snapshots to %s\n",
