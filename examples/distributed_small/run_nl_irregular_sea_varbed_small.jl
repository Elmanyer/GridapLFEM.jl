# ==============================================================
#  run_nl_irregular_sea_varbed_small.jl — SMALL-DOMAIN sea-state comparison run
#
#  CASE 16/16:  nonlinear · FULL pressure · VARIABLE bathymetry · irregular sea (BC) · Hs=0.2
#  Long-crested (normal-incidence) JONSWAP sea shoaling over a submerged bar
#  (flat_bed=false ⇒ ∇h terms ON) — the "real irregular sea over a shoal"
#  scenario. Compared with case 6 (flat bed, same sea) it isolates the
#  bathymetric shoaling and depth-induced harmonic release over the crest.
#  Generation stays at the offshore depth d0=3.5; the sea shoals to d≈2.0.
#
#  Bar identical to case 3 (y-invariant, analytic ∇h,∇²h): x_b=26, w=6, h_bar=1.5.
#  Geometry otherwise identical to case 6. Hs=0.2, Tp=2.0. y_wall_bc=:wall.
#
...
#        GridapLFEM.jl/examples/distributed_small/run_nl_irregular_sea_varbed_small.jl
#  (SLURM: run/dist_small/run_nl_irregular_sea_varbed_small.sh)
# ==============================================================

include(joinpath(@__DIR__, "..", "distributed", "_dist_common.jl"))

# ---- CASE 16 physics + long-crested sea defaults (env still overrides) ----
...
outdir   = genv("LFEM_OUTDIR", joinpath(ROOT, "output", "small_nl_irregular_sea_varbed_M$(M)"))

# smooth y-invariant submerged bar — same as case 3
sramp   = wbar / 3.0
h_bathy(x) = d0 - 0.5*hbar*(tanh((x[1]-(xbar-wbar))/sramp) - tanh((x[1]-(xbar+wbar))/sramp))

state = build_airy_state(d0; directional=false)

banner("SMALL | NONLINEAR full-pressure IRREGULAR sea over a BAR (Dirichlet BC, variable bed)",
       M, (px,py), (nx,ny), nx*ny, outdir)
is_rank0() && @printf("#   Hs=%.4g m Tp=%.3g s | bar d:%.1f→%.1f m | bc_side=%s seed=%d\n",
                      hs_val(), Tp, d0, d0-hbar, string(bc_side_sym()),
...
diags, vert, prob = setup_and_run_distributed(
    ...
    h_bathy=h_bathy,                                               # ★ variable bathymetry (submerged bar)
    regime=regime_sym(), nl_pressure=nl_pressure_sym(), flat_bed=flat_bed_flag(0),  # ★ CASE 16 (∇h ON; defaults above)
...
is_rank0() && @printf("small_nl_irregular_sea_varbed done: %d steps, %d snapshots to %s\n",
