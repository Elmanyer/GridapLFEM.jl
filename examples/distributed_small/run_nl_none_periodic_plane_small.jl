# ==============================================================
#  run_nl_none_periodic_plane_small.jl — SMALL-DOMAIN comparison run
#
#  CASE 7/12:  nonlinear · NO nonlinear pressure · flat bed · y-periodic · A=0.1
#  Same flat periodic plane wave as case 2, but nl_pressure=:none — the
#  nonlinear core (advection + full leading-pressure dispersion) WITHOUT the
#  eight 𝓝 non-hydrostatic-pressure components. Bottom of the pressure sweep
#  {none → native (case 8) → full (case 2)}: the differences isolate what each
#  nonlinear-pressure tier adds (the O(A²)–O(A³) bound harmonics).
#
...
# ---- CASE 7 physics config (env still overrides; keeps banner ⇔ solver consistent) ----
get!(ENV, "LFEM_REGIME", "nonlinear"); get!(ENV, "LFEM_NL_PRESSURE", "none")
...
outdir   = genv("LFEM_OUTDIR", joinpath(ROOT, "output", "small_nl_none_periodic_plane_M$(M)"))

banner("SMALL | NONLINEAR no-𝓝-pressure periodic plane wave (flat bed, A=$(Awave))",
...
diags, vert, prob = setup_and_run_distributed(
    ...
    regime=regime_sym(), nl_pressure=nl_pressure_sym(), flat_bed=flat_bed_flag(),  # ★ CASE 7 (defaults above)
...
is_rank0() && @printf("small_nl_none_periodic_plane done: %d steps, %d snapshots to %s\n",
