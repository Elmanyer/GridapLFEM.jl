# ==============================================================
#  run_lin_directional_sea_small.jl — SMALL-DOMAIN sea-state comparison run
#
#  CASE 13/16:  LINEAR · directional sea (BC gen) · flat bed · Hs=0.2
#  Short-crested JONSWAP × cosine-power sea generated by time-varying Dirichlet
#  data (η, 𝖴x, 𝖴y) on the LEFT boundary, propagated with the LINEAR dispersive
#  model. Compared with case 5 (nonlinear-full, same Hs) the difference is the
#  finite-amplitude sea-state physics (nonlinear harmonics, set-down, wave–wave
#  interaction); this run is the clean linear reference. Directional seas
#  prescribe 𝖴y ≠ 0, so y_wall_bc=:open (lateral sponges).
#
#  Geometry identical to case 5 (Dirichlet left, relaxation zone 6 m, right
#  sponge 12 m, lateral sponges 4 m). Hs=0.2, Tp=2.0.
#
...
#        GridapLFEM.jl/examples/distributed_small/run_lin_directional_sea_small.jl
#  (SLURM: run/dist_small/run_lin_directional_sea_small.sh)
# ==============================================================

include(joinpath(@__DIR__, "..", "distributed", "_dist_common.jl"))

# ---- CASE 13 physics + sea + small-box spreading defaults (env still overrides) ----
...
outdir   = genv("LFEM_OUTDIR", joinpath(ROOT, "output", "small_lin_directional_sea_M$(M)"))

state = build_airy_state(d; directional=true)

banner("SMALL | LINEAR DIRECTIONAL sea (Dirichlet BC, flat bed)",
       M, (px,py), (nx,ny), nx*ny, outdir)
is_rank0() && @printf("#   Hs=%.4g m Tp=%.3g s | nθ=%d σθ=%.1f° θmax=%.1f° seed=%d\n",
...
diags, vert, prob = setup_and_run_distributed(
    ...
    regime=regime_sym(), nl_pressure=nl_pressure_sym(), flat_bed=flat_bed_flag(),  # ★ CASE 13 (defaults above)
...
is_rank0() && @printf("small_lin_directional_sea done: %d steps, %d snapshots to %s\n",
