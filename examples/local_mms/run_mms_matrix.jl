...
#
#  ENV
#    BALFEM_CONV_PU      comma list of velocity orders   2,3,4   (⇒ Q2/Q1,Q3/Q2,Q4/Q3)
#    BALFEM_CONV_DOMAIN  d1 | d2                          d1
#    BALFEM_CONV_MODE    static | transient | both        both
#    BALFEM_CONV_LEVELS  refinement levels                4
#    BALFEM_CONV_NX0     coarsest nx                      8
#    BALFEM_CONV_NY0     coarsest ny (2-D only)           8
#    BALFEM_CONV_DT      time step                        1e-4
#    BALFEM_CONV_NSTEPS  steps                            100
#    BALFEM_CONV_DIST    0 | 1 distributed                0
...
pus    = parse.(Int, split(genv("BALFEM_CONV_PU","2,3,4"), ","))
domain = Symbol("d"*string(genv("BALFEM_CONV_DOMAIN","d1")[end]))
modes  = let m=genv("BALFEM_CONV_MODE","both")
    m=="both" ? [:static,:transient] : [Symbol(m)] end
levels = genv_i("BALFEM_CONV_LEVELS",4)
nx0    = genv_i("BALFEM_CONV_NX0",8); ny0 = genv_i("BALFEM_CONV_NY0",8)
dt     = genv_f("BALFEM_CONV_DT",1e-4); nsteps = genv_i("BALFEM_CONV_NSTEPS",100)
